...
export const projects: Project[] = [
    {
        ...
        troubleShooting: [
            ...
            },
            {
                ...
                details: [
                    '[문제점] 실시간으로 유입되는 대규모 시계열 데이터를 동기적으로 처리할 경우, 향후 데이터 처리량이 급증했을 때 시스템에 병목 현상이 발생하여 전체 안정성을 위협할 수 있다고 판단했습니다.',
                    '[해결] RabbitMQ를 선제적으로 도입하여 데이터 처리 요청을 메시지 큐에 적재하고, 별도의 컨슈머가 이를 비동기적으로 처리하는 파이프라인을 설계 및 구축했습니다. 이를 통해 시스템 부하를 효과적으로 분산시켜, 잠재적으로 발생할 수 있는 데이터 급증 상황에도 안정적으로 시스템을 운영할 수 있는 확장 가능한 기반을 마련했습니다.',
                ],
            },
        ],
        screenshots: [
        ...
        contributions: [
            {
                ...
                details: [
                    'OAuth 로그인 및 FCM 푸시 알림 등 Node.js 생태계에 친화적인 기능 구현을 가속화하고 개발 생산성을 높이기 위해, 기존 Spring Boot(Java) 서버의 모든 API 엔드포인트를 재작성하여 Nest.js(TypeScript) 기반으로 성공적인 마이그레이션을 주도했습니다. 특히 Spring Boot의 MVC 패턴과 유사한 Nest.js의 모듈 구조 덕분에 새로운 기술 스택에 빠르게 적응할 수 있었습니다.',
    ...
    {
        title: 'Midas Trader - 텔레그램 봇 기반 가상화폐 자동 매매 시스템',
        description:
            'Python의 데이터 분석 생태계와 FastAPI의 비동기 처리 성능을 결합하여 구축한 다중 사용자 기반 가상화폐 퀀트 트레이딩 플랫폼',
        tech: [
            'Python',
            'FastAPI',
            'PostgreSQL',
        ...
        thumbnail: '/projects/MidasTrader/thumbnail.png',
        category: [
            '백엔드',
            '퀀트·자동매매',
            'CCXT',
        ...
        detailedDescription:
            '다양한 가상화폐 거래소를 지원하는 다중 사용자 지원 고성능 자동 매매 시스템입니다. 데이터베이스 중심의 동적 거래소 연결 구조를 채택하여 확장성을 극대화했으며, Numba JIT 컴파일러와 비동기 설계를 통해 연산 레이턴시를 최소화하여 24시간 중단 없는 안정적인 퀀트 트레이딩 환경을 구축했습니다. 또한 텔레그램 봇을 통해 사용자 가입부터 API 키 등록, 매매 전략 설정까지 대화형으로 설정·관리할 수 있는 직관적인 인터페이스를 제공합니다.',
        features: [
            '다중 사용자 및 다중 거래소 지원 동적 연결 아키텍처',
            '파라미터·지표 기반 맞춤형 매매 전략 제공',
            '텔레그램 봇 기반의 대화형 시스템 운영 및 실시간 알림·리포트',
            '단일 루프 기반 다중 매매 전략 스케줄링 및 리소스 관리',
        ],
        contributions: [
            {
                title: '비동기 기반 고성능 자동 매매 엔진 및 스케줄러 구축',
                details: [
                    'APScheduler를 활용하여 메인 스레드 차단 없이 백그라운드에서 매 분 00초(Cron)마다 시장 데이터를 조회하고 전략을 실행하는 정밀한 타이밍 제어 시스템을 구축했습니다.',
                    '다수 사용자의 매매 전략을 병렬로 처리하기 위해 CCXT의 비동기 지원과 FastAPI를 결합하였으며, Numba JIT 컴파일러를 적용해 지표 연산 레이턴시를 획기적으로 최소화했습니다.',
                ],
            },
            {
                title: '확장성을 고려한 동적 거래소 연동 및 보안 시스템 설계',
                details: [
                    '사용자 및 거래소별 자격 증명 데이터를 기반으로 API 요청 시점에 CCXT 인스턴스를 동적으로 생성하고 관리하는 팩토리 구조를 도입해 다중 거래소 지원의 확장성을 확보했습니다.',
                    '사용자의 민감한 거래소 API Key 및 Secret Key를 Cryptography의 AES-256 알고리즘으로 안전하게 암호화하여 DB에 보관하는 보안 계층을 구현했습니다.',
                ],
            },
            {
                title: '데이터 무결성을 위한 상태 관리 및 중복 진입 방지 로직 구현',
                details: [
                    '모든 포지션 상태와 거래 이력을 PostgreSQL에 기록하고, 시그널 발생 캔들의 타임스탬프를 Unique Key로 활용하여 동일 시그널에 대한 중복 매매를 원천 차단했습니다.',
                ],
            },
            {
                title: '텔레그램 봇 기반 통합 운영 및 알림 시스템 개발',
                details: [
                    '웹 프론트엔드 대신 텔레그램 봇을 활용하여 회원가입, 거래소 연결, 매매 전략 설정, 기간별 승률 조회 등을 수행할 수 있는 대화형 UI·UX를 구축했습니다.',
                ],
            },
        ],
        troubleShooting: [
            {
                title: '시스템 재시작 및 스위칭 시 발생하는 "유령 포지션" 동기화 문제 해결',
                details: [
                    '[문제점] 24시간 가동되는 시스템 특성상, 예상치 못한 서버 재시작이나 포지션 스위칭 시 실제 거래소의 포지션 상태와 시스템 DB의 상태가 불일치하는 "유령 포지션" 현상이 발생할 위험이 있었습니다.',
                    '[해결] 포지션 스위칭 로직 실행 시, 단순히 DB 상태만 업데이트하는 것이 아니라 실제 거래소의 현재 포지션 상태를 API로 조회하여 DB와 엄격하게 대조하도록 구현했습니다. 불일치가 감지되면 자동으로 상태를 동기화하고 에러 핸들링을 수행하여 시스템의 정합성과 안정성을 확보했습니다.',
                ],
            },
            {
                title: '네트워크 불안정에 따른 API 호출 실패 및 데이터 누락 방지',
                details: [
                    '[문제점] 실시간 티커 및 캔들(OHLCV) 데이터를 비동기로 빈번하게 수집하는 과정에서, 거래소 API의 일시적인 응답 지연이나 네트워크 불안정으로 인해 데이터 조회가 실패하여 매매 시그널 판단에 치명적인 오류를 초래할 수 있었습니다.',
                    '[해결] 외부 API 의존성에서 발생하는 불가피한 실패를 보완하기 위해 Tenacity 라이브러리를 도입했습니다. API 요청 실패 시 자동 재시도(Retry) 로직을 적용하여, 일시적인 네트워크 장애 상황에서도 데이터 누락 없이 시스템이 복구 및 정상 동작하도록 안정성을 강화했습니다.',
                ],
            },
        ],
        screenshots: [
            {
                url: '/projects/MidasTrader/001.png',
                caption: '텔레그램 봇을 통한 대화형 매매 전략 설정 인터페이스',
            },
            {
                url: '/projects/MidasTrader/002.png',
                caption:
                    '포지션 진입·청산 및 실현 손익금(PnL) 상세 리포트 알림',
            },
            {
                url: '/projects/MidasTrader/003.png',
                caption: '기간 및 조건 기반 다차원 승률 조회 세부 리포트',
            },
        ],
    },
];
